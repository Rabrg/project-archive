...
public class Group {
	...
	public Group(final String name, final int level, final String color, final List<String> actions) {

		this.name = name;
		this.level = level;
		this.actions = actions;

		final ChatColor c = ChatColor.valueOf(color);
		if (c == null) {
	...
	public static Group load(String string) {
		try {
			if (string.split(":").length == 4) {
				...
				final String name = string.split(":")[0];
				final int level = Integer.parseInt(string.split(":")[1]);
				final String color = string.split(":")[2];
				final List<String> actions = Arrays.asList(string.split(":")[3].split(","));
