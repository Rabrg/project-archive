	...
	public int execute() {
		final Item food = ctx.getInventory().get(new Filter<Item>() {
			...
			public boolean match(final Item item) {
				return item != null && item.getName() != null && (item.getName().toLowerCase().contains("swordfish") || item.getName().toLowerCase().contains("salmon") || item.getName().toLowerCase().contains("cake"));
			}
		});
		if (food != null) {
			food.interact("Eat");
			nextHeal = Calculations.random(3, ctx.getSkills().getRealLevel(Skill.HITPOINTS) - 7);
		} else {
			ctx.getTabs().logout();
		}
