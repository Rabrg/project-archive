...
public class BankCleanHerbTaskNode extends TaskNode {
    ...
    public int execute() {
        ...
        if (!getBank().isOpen() && getBank().openClosest()) {
            ...
            grandExchange = true;
                ...
                && getBank().withdraw(buy[0].name, getInventory().getEmptySlots())) {
            sleepUntil(new Condition() {
                @Override
                public boolean verify() {
        ...
        } else if (getBank().isOpen() && getInventory().contains(buy[0].name) && getBank().close()) {
            sleepUntil(new Condition() {
                @Override
                public boolean verify() {
                    return !getBank().isOpen();
