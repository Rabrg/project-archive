...
public class Configuration {
	...
	private final ArrayList<ConfigurationFile> flatfiles;
	private final ArrayList<ConfigurationTable> dbfiles;

	@SuppressWarnings("serial")
	public Configuration(final AntiCheat plugin, final AntiCheatManager manager) {
		removeOldFiles();
		...
		for (final ConfigurationFile file : flatfiles) {
			file.save();
			checkReload(file);
		}
	}
	...
	public void load() {
		...
		for (final ConfigurationTable table : dbfiles) {
			table.load();
		}
	...
	private void checkReload(final ConfigurationFile file) {
		if (file.needsReload()) {
			file.reload();
	...
	private void removeOldFiles() {
		...
		final File configFile = new File(AntiCheat.getPlugin().getDataFolder(), "config.yml");
		...
		final File eventsFile = new File(AntiCheat.getPlugin().getDataFolder(), "events.yml");
