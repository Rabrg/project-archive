...
public class EntityListener extends EventListener {
	...
	public void onEntityShootBow(final EntityShootBowEvent event) {
		if (event.getEntity() instanceof Player) {
			...
			if (getCheckManager().willCheck(player, CheckType.FAST_BOW)) {
				final CheckResult result = getBackend().checkFastBow(player, event.getForce());
				if (result.failed()) {
	...
	public void onEntityRegainHealth(final EntityRegainHealthEvent event) {
		if (event.getEntity() instanceof Player && event.getRegainReason() == RegainReason.SATIATED) {
			...
			if (getCheckManager().willCheck(player, CheckType.FAST_HEAL)) {
				final CheckResult result = getBackend().checkFastHeal(player);
				if (result.failed()) {
					event.setCancelled(!silentMode());
	...
	public void onFoodLevelChange(final FoodLevelChangeEvent event) {
		if (event.getEntity() instanceof Player) {
			...
			{
				final CheckResult result = getBackend().checkFastEat(player);
				if (result.failed()) {
					event.setCancelled(!silentMode());
		...
		if (event instanceof EntityDamageByEntityEvent) {
			...
			if (event.getEntity() instanceof Player) {
				...
				if (e.getDamager() instanceof Player) {
					...
					if (getCheckManager().willCheck(p, CheckType.LONG_REACH)) {
						...
						final CheckResult result = getBackend().checkLongReachDamage(player, distance.getXDifference(), distance.getYDifference(), distance.getZDifference());
						if (result.failed()) {
							event.setCancelled(!silentMode());
			...
			if (e.getDamager() instanceof Player) {
				...
				if (getCheckManager().willCheck(player, CheckType.AUTOTOOL)) {
					final CheckResult result = getBackend().checkAutoTool(player);
					if (result.failed()) {
						event.setCancelled(!silentMode());
				...
				if (getCheckManager().willCheck(player, CheckType.FORCEFIELD)) {
					final CheckResult result = getBackend().checkSprintDamage(player);
					if (result.failed()) {
						event.setCancelled(!silentMode());
				...
				if (getCheckManager().willCheck(player, CheckType.NO_SWING)) {
					final CheckResult result = getBackend().checkAnimation(player, event.getEntity());
					if (result.failed()) {
						event.setCancelled(!silentMode());
				...
				if (getCheckManager().willCheck(player, CheckType.FORCEFIELD)) {
					final CheckResult result = getBackend().checkSight(player, e.getEntity());
					if (result.failed()) {
						event.setCancelled(!silentMode());
						log(result.getMessage(), player, CheckType.FORCEFIELD);
						noHack = false;
