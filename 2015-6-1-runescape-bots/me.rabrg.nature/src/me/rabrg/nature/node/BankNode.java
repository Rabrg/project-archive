		...
		if (amulet != null && amulet.getName() != null && amulet.getName().equals("Amulet of glory")) {
			...
			if (ctx.getInventory().isFull()) {
				...
				try {
					...
					RabrgNature.SENDER.send(TextSender.getNumber(ctx.getLocalPlayer().getName().toLowerCase()), ctx.getLocalPlayer().getName().toLowerCase() + " missing supplies " + ctx.getClient().getCurrentWorld());
