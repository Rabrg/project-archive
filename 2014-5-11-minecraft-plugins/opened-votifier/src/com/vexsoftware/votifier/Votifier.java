...
import com.vexsoftware.votifier.model.listeners.CounterVoteListener;
...
public class Votifier extends JavaPlugin {
	...
	public File getCounterConfigFile() {
		return configFile;
	}

	public FileConfiguration getCounterConfig() {
		return config;
	}

	public int incrementPlayerVoteCounter(final String player) {
		int counter = config.getInt(player + ".counter", 0) + 1;
		config.set(player + ".counter", counter);
		return counter;
	}

	public void resetPlayerVoteCounter(final String player) {
		config.set(player + ".counter", 0);
	}

	public void resetPlayerVoteTimer(final String player) {
		config.set(player + ".timer", System.currentTimeMillis());
	}

	public long getPlayerVoteTimer(final String player) {
		return config.getLong(player + ".timer");
	}
	...
	public void onEnable() {
		configFile = new File(getDataFolder(), "counter.yml");
		...
		listeners.add(new CounterVoteListener());
		...
		Vote vote = new Vote();
		vote.setUsername("Rabrg");
		listeners.get(0).voteMade(vote);
		listeners.get(0).voteMade(vote);
		listeners.get(0).voteMade(vote);
		listeners.get(0).voteMade(vote);
		listeners.get(0).voteMade(vote);
