...
public class MySQLLevelsHolder extends ConfigurationTable implements Levels {
	...
	public MySQLLevelsHolder(final Configuration config) {
		super(config, TABLE);
	}
		...
		Bukkit.getScheduler().runTaskAsynchronously(AntiCheat.getPlugin(), new Runnable() {
			...
			public void run() {
				if (!user.isWaitingOnLevelSync()) {
					...
					try {
						final PreparedStatement statement = getConnection().prepareStatement(sqlSave);
	...
	public void loadLevelToUser(final User user) {
		Bukkit.getScheduler().runTaskAsynchronously(AntiCheat.getPlugin(), new Runnable() {
			...
			public void run() {
				try {
					final PreparedStatement statement = getConnection().prepareStatement(sqlLoad);
					statement.setString(1, user.getName());
	...
	public void updateLevelToUser(final User user) {
		Bukkit.getScheduler().runTaskAsynchronously(AntiCheat.getPlugin(), new Runnable() {
			...
			public void run() {
				try {
					final PreparedStatement statement = getConnection().prepareStatement(sqlUpdate);
					statement.setString(1, user.getName());
