...
public class YamlGroupsHolder extends ConfigurationFile implements Groups {
	...
	public YamlGroupsHolder(final AntiCheat plugin, final Configuration config) {
	...
	@Override
	public void open() {
		final ConfigValue<List<String>> groups = new ConfigValue<List<String>>("groups");

		// Convert groups list to Levels
		this.groups = new ArrayList<Group>();
		for (final String string : groups.getValue()) {
			final Group group = Group.load(string);
		...
		Collections.sort(this.groups, new Comparator<Group>() {
			@Override
			public int compare(final Group l1, final Group l2) {
