...
public final class FishNode extends Node {
	...
	public boolean validate() {
		return !ctx.getInventory().isFull() && (((fishingSpot = ctx.getNpcs().closest(FISHING_SPOT_FILTER)) != null && fishingSpot.distance() > 1 ) || ctx.getLocalPlayer().getAnimation() != 621);
	}
		...
		if (fishingSpot.interact("Net")) {
			MethodProvider.sleepUntil(new Condition() {
				...
				public boolean verify() {
					return ctx.getLocalPlayer().getAnimation() == 621;
				}
