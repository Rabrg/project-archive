...
public final class Gap3Node extends VarrockNode {

	private GameObject gap;

	public Gap3Node(final MethodContext ctx) {
		super(ctx);
	}

	@Override
	public boolean validate() {
		return GAP_3_AREA.contains(ctx.getLocalPlayer()) && (gap = ctx.getGameObjects().getTopObjectOnTile(new Tile(3209, 3398, 3))) != null;
	}
		...
		if (gap.distance() > 5) {
			...
			MethodProvider.sleepUntil(new Condition() {
				...
				public boolean verify() {
					return !GAP_3_AREA.contains(ctx.getLocalPlayer());
				}
