...
@ScriptManifest(author = "Rabrg", category = Category.WOODCUTTING, name = "Oak cut", version = 0.1)
public final class OakCut extends AbstractScript {
	...
	public int onLoop() {
		if (getInventory().isFull()) {
			...
			final GameObject tree = getGameObjects().closest("Oak");
			if (tree != null) {
				if (tree.interactForceLeft("Chop down")) {
					sleepUntil(new Condition() {
						@Override
						public boolean verify() {
							return !getGameObjects().getTopObjectOnTile(tree.getTile()).getName().equals("Oak");
						}
